...
exist.
On X11 builds with Xft (\texttt{+XFT} in \texttt{unicon -features}),
2D windows render these names with antialiased scalable fonts via
Xft/fontconfig/FreeType; without Xft, core X fonts are used.
On the OpenGL graphics path, FreeType may load a TrueType or OpenType
font \emph{file} as the family field (for example
\texttt{mymono.ttf,18}); see Chapter~7 and UTR~22.
