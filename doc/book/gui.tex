...
(\texttt{theme.icn}) so applications can choose a modern flat
...
\noindent
Recognized names include \texttt{flat} (aliases \texttt{modern},
\texttt{default}) and \texttt{classic} (aliases \texttt{bevel},
\texttt{motif}). Flat is the package default if nothing is set.
\texttt{GetGuiTheme()} and \texttt{GuiThemeIsFlat()} report the
active choice.

\paragraph{Fonts and colors}

Flat theme dialogs pick default window attributes (background,
foreground, and font) via \texttt{GuiThemeDialogAttribs()}; explicit
dialog attributes still override those defaults. Flat face and input
...
\iconcode{
SetGuiThemeFont("default")\ \ \# portable Unicon sans,12 \\
SetGuiThemeFont("modern")\ \ \ \# Arial/helvetica size 13 when available
}

\noindent
\texttt{GuiThemePickFont()} returns the concrete \texttt{font=}
string for the active font style. Optional colors are carried in a
