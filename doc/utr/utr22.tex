...
This FreeType path applies to the OpenGL-based graphics implementation
described in this report. The classic X11 2D backend
(\texttt{open(..., "g")} without OpenGL 2D) is separate: when Unicon
is built with Xft (\texttt{+XFT} in \texttt{unicon -features}), that
backend draws antialiased text with Xft, fontconfig, and FreeType as
the usual system rasterizer, rather than loading \texttt{.ttf} files
through the OpenGL font API. See the Fonts section of the Unicon book
and UTR~21 for build options.
